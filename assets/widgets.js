/* learn-github-for-builders-with-phoebe - interactive widgets (CLI + git graphs)
...
(function () {
  ...
  var C = { deep: "#1B1F23", prim: "#0969DA", mid: "#2DA44E", soft: "#86D9A3",
            tint: "#DAFBE1", ink: "#1F2328", muted: "#656D76", flag: "#8250DF",
            good: "#1F883D", bad: "#B42318" };
  ...
  /* ---------- 12 · gitplay : clickable commit-graph simulator ---------- */
  function wGitPlay(root, c) {
    // model: nodes = {id, x, row, branch, parents:[ids], label}
    // rows: 0 = main, 1 = feature (and higher for extra branches)
    root.appendChild(el("p", "wg-title", c.title || "★ Git graph playground - build history one command at a time"));
    if (c.intro) root.appendChild(el("p", "wg-note", c.intro));
    var branchColors = [C.prim, C.flag, C.good, "#BC4C00"];
    var branchNames = ["main"];
    var nodes = [];
    var heads = { main: null };     // branch name -> latest node id
    var current = "main";
    var nextX = 1, nextId = 1;
    var log = el("p", "wg-note", "");
    var svgWrap = el("div", "wg-svgwrap");

    function addCommit() {
      var parent = heads[current];
      var row = branchNames.indexOf(current);
      var n = { id: nextId++, x: nextX++, row: row, branch: current, parents: parent === null ? [] : [parent], label: "c" + (nodes.length + 1) };
      nodes.push(n); heads[current] = n.id;
      say("commit on " + current);
    }
    function newBranch() {
      if (branchNames.length >= 4) { say("keep it to 4 branches for the demo"); return; }
      var name = ["feature", "hotfix", "release"][branchNames.length - 1] || ("branch" + branchNames.length);
      branchNames.push(name);
      heads[name] = heads[current];   // branch points at current commit
      current = name;
      say("branch " + name + " created and checked out - no commit yet");
    }
    function switchBranch() {
      var i = (branchNames.indexOf(current) + 1) % branchNames.length;
      current = branchNames[i];
      say("switched to " + current);
    }
    function merge() {
      if (current === "main") { say("switch to main first, then merge a branch into it"); return; }
      var from = current, into = "main";
      var p1 = heads[into], p2 = heads[from];
      if (p1 === p2) { say("nothing to merge - branches are level"); return; }
      var n = { id: nextId++, x: nextX++, row: 0, branch: into, parents: [p1, p2], label: "M", merge: true };
      nodes.push(n); heads[into] = n.id; current = into;
      say("merged " + from + " into main - see the two parent lines join");
    }
    function reset() {
      branchNames = ["main"]; nodes = []; heads = { main: null }; current = "main"; nextX = 1; nextId = 1;
      addCommit(); draw();
    }
    function nodeById(id) { for (var i = 0; i < nodes.length; i++) if (nodes[i].id === id) return nodes[i]; return null; }
    function say(msg) { log.innerHTML = "<strong>HEAD → " + current + "</strong> &nbsp;·&nbsp; " + msg; draw(); }

    function draw() {
      var W = 760, rowH = 62, padL = 40, padT = 30, colW = 84;
      var maxRow = branchNames.length - 1;
      var H = padT + maxRow * rowH + 90;
      svgWrap.innerHTML = "";
      var svg = svgEl("svg", { viewBox: "0 0 " + W + " " + H, role: "img", "aria-label": "git commit graph" });
      function X(x) { return padL + x * colW; }
      function Y(row) { return padT + row * rowH; }
      // branch lane labels
      branchNames.forEach(function (b, r) {
        var t = svgEl("text", { x: 4, y: Y(r) + 5, fill: branchColors[r], "font-size": "12.5", "font-weight": "700", "font-family": "Inter, sans-serif" });
        t.textContent = b; svg.appendChild(t);
      });
      // edges
      nodes.forEach(function (n) {
        n.parents.forEach(function (pid) {
          var p = nodeById(pid); if (!p) return;
          var col = branchColors[n.merge && p.row !== n.row ? p.row : n.row];
          svg.appendChild(svgEl("path", {
            d: "M" + X(p.x) + " " + Y(p.row) + " C" + (X(p.x) + colW / 2) + " " + Y(p.row) + " " + (X(n.x) - colW / 2) + " " + Y(n.row) + " " + X(n.x) + " " + Y(n.row),
            fill: "none", stroke: col, "stroke-width": 3, opacity: .8
          }));
        });
      });
      // nodes
      nodes.forEach(function (n) {
        var isHead = heads[current] === n.id;
        svg.appendChild(svgEl("circle", { cx: X(n.x), cy: Y(n.row), r: n.merge ? 11 : 9, fill: branchColors[n.row], stroke: "#fff", "stroke-width": 2 }));
        if (isHead) svg.appendChild(svgEl("circle", { cx: X(n.x), cy: Y(n.row), r: 15, fill: "none", stroke: C.flag, "stroke-width": 2 }));
      });
      svgWrap.appendChild(svg);
    }

    var btns = el("div", "wg-btnrow");
    [["+ commit", addCommit], ["+ branch", newBranch], ["switch", switchBranch], ["merge to main", merge]].forEach(function (b) {
      var btn = el("button", "wg-btn big", b[0]); btn.type = "button";
      btn.addEventListener("click", function () { b[1](); });
      btns.appendChild(btn);
    });
    var rbtn = el("button", "wg-btn", "Reset"); rbtn.type = "button";
    rbtn.addEventListener("click", reset); btns.appendChild(rbtn);

    root.appendChild(svgWrap); root.appendChild(btns); root.appendChild(log);
    root.appendChild(el("p", "wg-note", c.note || "This is exactly what git does under the hood: commits are snapshots, a branch is a movable pointer, HEAD is where you are standing, and a merge commit has two parents."));
    reset();
  }

  /* ---------- boot ---------- */
  var registry = { sorter: wSorter, mdpreview: wMdPreview, checklist: wChecklist, gitplay: wGitPlay, meanmedian: wMeanMedian, kpitree: wKpiTree, funnel: wFunnel,
